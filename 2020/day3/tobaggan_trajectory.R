...
tree_slope <- readLines("2020/day3/tree_slope.txt")
...
tree_slope_big <- matrix(rep(tree_slope, hill_size), byrow=FALSE, ncol=hill_size*ncol(tree_slope))

x <- 1
y <- 1

while(y<=323){
  if(tree_slope_big[y,x]=="#"){
    tree_slope_big[y,x]="X"
  } else{
    tree_slope_big[y,x]="O"
  }
  x = x+steps_right
  y = y+steps_down
}
return(sum(tree_slope_big=="X"))
}
